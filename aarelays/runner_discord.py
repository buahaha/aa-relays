...
from aarelays.models import AccessTokens, RelayConfigurations, Channels, \
    Messages, Servers
...
async def on_ready():
    ...
    if runnertoken.appear_offline:
        await client.change_presence(status=discord.Status.offline, afk=True)
    ...
    for guild in list(client.guilds):
        ...
        Servers.objects.update_or_create(server=guild.id,
                                         defaults={'name': guild.name, 'protocol': "Discord"})
...
async def on_message(message):
    ...
    for relayconfiguration in relayconfigurations:
        logger.debug("Message Event Received")
        if relayconfiguration.source_server.filter(server=message.channel.guild.id) \
                or relayconfiguration.source_server_all is True:
            logger.debug("Matched Server {}".format(message.channel.guild.id))
            if relayconfiguration.source_channel.filter(channel=message.channel.id) \
                    or relayconfiguration.source_channel_all is True:
                logger.debug("Matched Channel {}".format(message.channel.id))

                # Its important to collate the whole message here
                # or we miss embeds and attachments
                all_content = [f"```\n{message.content}\n```"]
                ...
                # finally lets run our regex over the entire joined message
                # plus headers in case people want to be fancy
                # most people will use the Mention bools tho
                ...
                if message.mention_everyone == relayconfiguration.message_mention \
                        ...
                        or re.search(relayconfiguration.message_regex.string, joined_content_with_headers) is not None:
                    ...
                    if relayconfiguration.attempt_translation is True:
                        try:
                    ...
                    if relayconfiguration.destination_db is True:
                        logger.debug("DB Logging Message")
                        try:
                            Messages.objects.create(message=message.id,
                    ...
                    if relayconfiguration.destination_webhook.all() is not None:
